...
static inline u64 scale_exec_time(u64 delta, struct rq *rq)
{
	...
	/*
	 * The 9R 4.19 tree stores a combined scale in rq->task_exec_scale.
	 * The msmnile 4.14 WALT tree keeps the equivalent inputs on the
	 * scheduler cluster instead.  Use those lockless tick-path fields.
	 */
	if (!rq->cluster || !rq->cluster->max_possible_freq)
		return delta;
	delta = div64_u64(delta * rq->cluster->cur_freq,
			rq->cluster->max_possible_freq);
	return (delta * rq->cluster->exec_scale_factor) >> 10;
#else
...
static int cpu_info_dump_win(struct seq_file *m, void *v, u32 win_cnt)
{
	...
	for (i = 0; i < win_cnt; i++) {
		...
		/*
		 * Keep the donor's three hot-thread columns in the composite ABI.
		 * Their stack/task_struct producer is deliberately deferred from this
		 * containment batch, so report the donor's "no valid sample" token.
		 */
		seq_puts(m, "- - -");
