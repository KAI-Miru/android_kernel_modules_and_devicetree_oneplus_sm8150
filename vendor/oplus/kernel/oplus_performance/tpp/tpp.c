...
static bool tpp_topology_ready;
static bool record_system_cpu_state(void);
...
static int tpp_on_set(const char *buf, const struct kernel_param *kp)
{
	int val;
	...
	if (ret)
		return ret;
	if (val && !tpp_topology_ready && !record_system_cpu_state())
		return -EAGAIN;

	WRITE_ONCE(tpp_on, !!val);
	return 0;
}

static int tpp_on_get(char *buf, const struct kernel_param *kp)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", READ_ONCE(tpp_on));
}

static struct kernel_param_ops tpp_on_ops = {
	.set = tpp_on_set,
	.get = tpp_on_get,
};
...
module_param_cb(tpp_on, &tpp_on_ops, NULL, 0664);
...
static bool record_system_cpu_state(void)
{
	...
	tpp_cluster_size = 0;
	memset(tpp_cpu_clusters, 0, sizeof(tpp_cpu_clusters));
	while (cpu < NR_CPUS) {
		...
		if (policy == NULL) {
			...
			tpp_on = 0;
			tpp_topology_ready = false;
			return false;
		}
		cpu = save_cpu_cluster_and_ret_last_cpu(policy);
		cpu += 1;
	}
	tpp_topology_ready = (tpp_cluster_size == TPP_MAX_CLUSTER_SIZE);
	if (!tpp_topology_ready)
		tpp_loge("expected %d CPU clusters, found %u",
			TPP_MAX_CLUSTER_SIZE, tpp_cluster_size);
	return tpp_topology_ready;
}
...
/* H.40 registers the msmnile cpufreq driver at subsys_initcall time. */
late_initcall(tpp_init);
